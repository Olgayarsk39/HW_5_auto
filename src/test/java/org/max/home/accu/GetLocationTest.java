...
public class GetLocationTest extends AbstractTest{
    ...
    @Test
    void get_AdministrativeArea() throws IOException, URISyntaxException {
        logger.info("Тест код ответ 200 запущен");
        //given
        logger.debug("Формирование мока для GET /locations/v2/area/autocomplete");
        stubFor(get(urlPathEqualTo("/locations/v2/area/autocomplete"))
                .withQueryParam("apiKey", containing("82c9229354f849e78efe010d94150807"))
                .withQueryParam("q", notMatching("data"))
                .willReturn(aResponse()
                        .withStatus(200).withBody("OK")));
        CloseableHttpClient httpClient = HttpClients.createDefault();
        HttpGet request = new HttpGet(getBaseUrl()+"/locations/v2/area/autocomplete");
        URI uri = new URIBuilder(request.getURI())
                .addParameter("apiKey", "A_82c9229354f849e78efe010d94150807")
                .addParameter("q", ".data")
                .build();
        ...
        HttpResponse response = httpClient.execute(request);
        //then
        verify(getRequestedFor(urlPathEqualTo("/locations/v2/area/autocomplete")));
        assertEquals(200, response.getStatusLine().getStatusCode());
        assertEquals("OK", convertResponseToString(response));
    }

    @Test
    void get_AdministrativeArea_Header() throws IOException, URISyntaxException {
        //given
        stubFor(get(urlPathEqualTo("/locations/v1/cities/autocomplete"))
